...
<div class="container py-4">
  <div class="position-relative rounded-4 overflow-hidden shadow tarjeta-editar-usuario border border-2">
    ...
    <div class="encabezado-edicion text-white px-4 py-3">
      <h5 class="titulo-edicion"><i class="bi bi-pencil-square me-2"></i>Editar usuario</h5>
    </div>
    ...
    <div class="p-4 bg-white">
      ...
      <form method="POST" enctype="multipart/form-data"
        action="<?= RUTA_URL ?>/UsuariosControlador/editar/<?= $datos['usuario']->id_usuario ?>">

        <!-- Nombre y Email + Imagen -->
        <div class="row g-4 mb-4">
          ...
          <div class="col-md-4 text-center">
            <label class="form-label label-edicion d-block">Imagen actual</label>
            <img src="<?= RUTA_URL ?>/public/img/usuarios/<?= $datos['usuario']->imagen ?>"
              class="rounded-circle border shadow-sm mb-2" width="80" height="80" alt="Imagen">
        ...
        <div class="row g-4 mb-4">
          <div class="col-md-4">
            <label class="form-label label-edicion">Cambiar contraseña</label>
          ...
          <div class="col-md-4">
            ...
            <select name="id_rol" class="form-select rounded-pill" required>
              <?php foreach ($datos['roles'] as $rol): ?>
                <option value="<?= $rol->id_rol ?>" <?= ((isset($datos['valores']['id_rol']) && $datos['valores']['id_rol'] == $rol->id_rol) || (!isset($datos['valores']['id_rol']) && $rol->id_rol == $datos['usuario']->id_rol)) ? 'selected' : '' ?>>
                  <?= $rol->nombre_rol ?>
                </option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="col-md-4">
            ...
            <select name="id_departamento" class="form-select rounded-pill" required>
              <?php foreach ($datos['departamentos'] as $dep): ?>
                <option value="<?= $dep->id_departamento ?>" <?= ((isset($datos['valores']['id_departamento']) && $datos['valores']['id_departamento'] == $dep->id_departamento) || (!isset($datos['valores']['id_departamento']) && $dep->id_departamento == $datos['usuario']->id_departamento)) ? 'selected' : '' ?>>
                  <?= $dep->nombre ?>
                </option>
              <?php endforeach; ?>
            </select>
        ...
        <div class="switch-activo form-switch mb-4">
          ...
          <input class="form-check-input ms-2" type="checkbox" role="switch" name="activo" id="activo"
            <?= ((isset($datos['valores']['activo']) && $datos['valores']['activo']) || (!isset($datos['valores']['activo']) && $datos['usuario']->activo)) ? 'checked' : '' ?>>
        </div>
